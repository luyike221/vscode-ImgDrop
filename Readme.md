...
```bash
code --install-extension md-image-paste-1.0.0.vsix
```
...
在 VSCode 设置（`Ctrl+,`）中搜索 `mdImagePaste`。
...
// settings.json
{
  "mdImagePaste.saveDirectory": "${fileDir}/assets/${year}/${month}",
  "mdImagePaste.fileNamePattern": "${fileName}_${time}",
  "mdImagePaste.imageFormat": "jpg",
  "mdImagePaste.jpgQuality": 90,
  "mdImagePaste.mdLinkStyle": "absolute"
}
```
...
- **剪贴板读取**：Windows 用 PowerShell；macOS 优先 `pngpaste`；Linux 用 `xclip` / `xsel`
- **无图片时**：回退到 `editor.action.clipboardPasteAction`，正常粘贴文字
